            ...
            .addOnSuccessListener { result ->
                for (document in result) { // 가져온 문서들이 result로 들어감
                    ...
                    if(user_data.email.equals(intent.getStringExtra("id"))) { // 로그인한 유저의 데이터베이스 찾기
                        ...
                        // 시간 감시는 최초접속시 운동시에 운동 당일 날짜보다 다음날 데이터가 커야한다.
                        // 이 조건에 위반될시 이는 접속날짜(time)가 저번 다음운동날짜(nextTime)보다 최신이라는 의미이다.(ex 11.15일에 접속 저번운동 날짜11.13 위반발생 초기화시작)
                        ...
                        // 위의 위반 사례에 의해 날짜 데이터(nextTime)가 변경된다면 다음날이 되었다는 의미로 해당 변경과 함께 카운트(count)가 다시 0으로 초기화 된다.
                        val now : String = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd"))
                        if((now.toString()==user_data.time.toString())&&(user_data.count >=2)){//DB와 운동하는 현재시간 날짜 체크 같다면 DB의 카운트를 봄
                        ...
                        else if((user_data.nextTime.toInt()<=(now.toInt()))&&(user_data.count>=0)){//현재 시간이 경과하여 다음날 DB데이터 nextTime보다 클때 카운트 초기화 및 다음날 데이터 다시 세팅
                            ...
                            cal2.time = Date()//현재시간
                            if(intent.getStringExtra("id")==user_data.email) {//로그인한 아이디로 수행
                        ...
                        else {//위의 조건들이 아니라면 카우트가 2이하고 날짜 조건이 같은날인 것임
